...
The site uses a file-based approach with markdown files stored in the `content/` directory:
...
Content is managed by manually editing markdown files in the `content/` directory.
...
**For detailed instructions on adding new projects, see:** [`docs/ADDING_PROJECTS.md`](docs/ADDING_PROJECTS.md)

**Quick summary:**
1. Create/edit markdown files in `content/projects/`
2. Upload media to `public/images/uploads/[project-slug]/`
3. Ensure all required frontmatter fields are present (title, slug, shortSummary, mainSummary, year, services, featured, order)
4. Set `order` field to control display sequence (lower numbers appear first)
5. Set `featured: true` to show on homepage
6. Media items (images/videos) should have `order` fields for gallery sequence
7. Commit and deploy
...
- Content managed manually via markdown files in `content/` directory
...
4. **File-based Content** - Markdown-driven content management in `content/` directory
